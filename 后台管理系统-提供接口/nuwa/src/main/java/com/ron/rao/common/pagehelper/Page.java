...
//		String orderByAsc = request.getParameter("order[0][dir]");
//		String orderTarget = request.getParameter("order[0][column]");
//		//有些字段与数据库中的存储不一致，为了兼容排序，需要使用name传递实际数据库中的字段
//		String orderByName = request.getParameter("columns["+ orderTarget +"][name]");
//		if(orderByName == null || "".equals(orderByName)){
//			orderByName = request.getParameter("columns["+ orderTarget +"][data]");
//		}
//		
//		orderBy = " order by " + orderByName + " "+ orderByAsc;
		//分页查询可根据多个字段查询
		String orderByAsc = null;
		String orderTarget = null;
		String orderByName = "";
		int i=0;
		do {
			orderByAsc = request.getParameter("order["+i+"][dir]");
			orderTarget = request.getParameter("order["+i+"][column]");
			if (orderTarget!=null&&!"".equals(orderTarget)) {
				//有些字段与数据库中的存储不一致，为了兼容排序，需要使用name传递实际数据库中的字段
				orderByName =  request.getParameter("columns["+ orderTarget +"][name]");
				if (orderByName == null || "".equals(orderByName)) {
					orderByName = request.getParameter("columns["+ orderTarget +"][data]");
				}
		...
		} while (orderByAsc!=null);
		if (orderBy.length()>0) {
			orderBy = "order by "+orderBy.substring(0, orderBy.length()-1);
		}
		isPage = true;

		if (StringUtils.isNumber(pageLength)&& StringUtils.isNumber(iDisplayStart)){
			this.setPageSize(Integer.parseInt(pageLength));
			this.setPageNo(Integer.parseInt(iDisplayStart) / Integer.parseInt(pageLength) + 1);
			this.setDraw(Integer.parseInt(drawStr));
			this.setMessage(searchValue);
		}
